...
export class DatatableComponent extends FGenerico implements OnInit, OnChanges {
	...
	areAllSelected(): boolean {
		const checkboxColumnKey = 'id_codigos';
		return this.paginatedItems.every((item: any) => this.isCheckboxSelected(item[checkboxColumnKey]));
	}
	...
	toggleSelectAll(event: any): void {
		...
		if (event.target.checked) {
			this.paginatedItems.forEach((item: any) => {
				if (!this.selectedCheckboxes.includes(item[checkboxColumnKey])) {
					this.selectedCheckboxes.push(item[checkboxColumnKey]);
				}
			});
		} else {
			this.paginatedItems.forEach((item: any) => {
				const index = this.selectedCheckboxes.indexOf(item[checkboxColumnKey]);
				if (index !== -1) {
					this.selectedCheckboxes.splice(index, 1);
				}
			});
